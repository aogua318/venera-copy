...
- **设为封面**：本地漫画阅读页可一键将当前页设为漫画封面，立即生效
- **保持屏幕常亮**：自动播放（滚动/定时翻页）期间自动保持屏幕常亮，停止后恢复
- **末页判定优化**：连续模式下倒数第二屏即视为章末（末页常因高度不足无法完整显示）

### 外设支持

- **手柄/键盘/媒体键控制**（Android）：蓝牙或 USB 手柄、键盘、耳机线控均可控制阅读
  - 可映射动作：上一页、下一页、开/关自动播放、上一章、下一章、返回
...
- **独立书架**：导航面板新增书架页，支持与首页一致的本地漫画导入流程（带导入进度条，自动跳过同名漫画），导入后自动加入书架
- **结构化导入**：自动识别压缩包 / 目录的嵌套结构——多层单链嵌套自动拍平为单章，分支结构自动识别为多章节漫画（章节按文件夹名排序），目录导入格式与合并结果一致
- **多种排序**：添加时间 / 名称 / 最近阅读时间，支持正序与逆序，排序设置持久化
- **三种视图**：列表（原版卡片视图）/ 网格（统一尺寸 + 文件名）/ 瀑布流（按封面真实宽高比）
- **多选操作**：长按进入选择模式，支持全选 / 反选 / 全不选、移出书架、删除（本地漫画连同磁盘文件，带进度显示）
...
前往 [Releases](https://github.com/aogua318/venera-copy/releases) 页面下载最新版本：

| 文件 | 适用 |
|---|---|
| `venera-<版本>-arm64-v8a.apk` | 绝大多数现代手机 |
| `venera-<版本>-armeabi-v7a.apk` | 老旧 32 位设备 |
| `venera-<版本>-x86_64.apk` | 模拟器 / x86 设备 |
| `venera-<版本>.apk` | 通用版 |
