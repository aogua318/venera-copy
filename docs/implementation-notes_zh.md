...
| 文件 | 说明 |
|---|---|
| `lib/utils/comic_structure.dart` | 漫画目录结构分析：单链下钻、分支判定、递归图片收集、自然排序 |

### 12.2 结构化导入

- 压缩包导入（`CBZ.import`）与目录导入（`_checkSingleComic`）共享 `analyzeComicStructure`：单链下钻定位内容根；内容根 ≥2 个子目录 → 多章节漫画，否则单章。
- 多章节导入格式与漫画合并结果一致：第一层漫画名目录，第二层 `0,1,2...` 章节目录，章节名 = 原文件夹名（自然排序）；压缩包导入时文件从解压缓存**移动**。
- 目录导入取消"章节内含子目录即无效"的规则；漫画 `directory` 指向下钻后的内容根。
- 兼容保留 metadata.json 章节定义（start/end 拆分）。

### 12.3 本地阅读递归化

`LocalManager.getImages` 改为递归收集（排除 `cover.*` 与隐藏文件），`sortComicFiles` 自然排序。旧格式漫画行为不变。

### 12.4 合并命名修复

第一本漫画 `directory` 为绝对路径时仅取最后一段作为合并命名前缀，修复"完整路径+漫画名"问题。

### 12.5 同版本其他改动

- 滚动速度设置：滑杆 + 点击数值手动输入（`_AutoScrollSpeedSetting`，输入范围 100–600000 ms，滑杆范围 1000–60000）
- 书架视图模式选择后对话框自动关闭
- 导入 / 合并 / 删除长操作保持屏幕常亮（`utils/io.dart` 新增通用 `setScreenOn` 助手）
- 本地删除进度框顺序修复（先关确认框再弹进度框）
